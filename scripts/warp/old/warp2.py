...
def get_leap_model(mujoco_path: Optional[Path] = None, batch_dim: int = 1):
    ...
    # List cube ids.
    cube_ids = [ii for ii in range(len(model.body_name)) if model.body_name[ii] == "cube"]
...
def get_cube_model(cube_size: float = 0.035, batch_dim: int = 1):
    ...
    cube0 = cube_builder.add_body()
    cube_builder.add_joint_free(cube0)
    cube_builder.add_shape_mesh(cube0, mesh=cube_mesh, scale=wp.vec3(cube_size, cube_size, cube_size))
    ...
    model.cube_ids = wp.from_numpy(model.bodies_per_env * np.arange(batch_dim), dtype=int)
...
if __name__ == "__main__":
    ...
    # Get contact points.
    contact_points_cube = wp.zeros(shape=(batch_size, model.rigid_contact_max), dtype=wp.vec3, device="cuda")

    contact_points_other = wp.zeros(shape=(batch_size, model.rigid_contact_max), dtype=wp.vec3, device="cuda")

    sdf_vals = wp.zeros(shape=(batch_size, model.rigid_contact_max), dtype=float, device="cuda")
